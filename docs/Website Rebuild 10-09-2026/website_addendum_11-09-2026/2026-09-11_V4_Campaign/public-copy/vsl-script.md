...
For example, if a checker rejects an output and the workflow keeps trying the same thing, I would ask what new evidence the next attempt has. That question may lead us to a different recovery path rather than another retry. We can make the change and examine what happens.
